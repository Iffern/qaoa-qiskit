...
from qiskit_ibm_runtime import QiskitRuntimeService, IBMBackend, Options, Session, Sampler
from qiskit_optimization import QuadraticProgram, QiskitOptimizationError
from qiskit_optimization.algorithms import OptimizationResult, OptimizationResultStatus, MinimumEigenOptimizationResult, \
    OptimizationAlgorithm, MinimumEigenOptimizer
    ...
    def solve(self, problem: QuadraticProgram) -> OptimizationResult:
        ...
        while problem_.get_num_vars() > self._min_num_vars:
            ...
            with Session(service=self._service, backend=self._backend) as session:
                if hasattr(self._optimizer, 'qaoa'):
                    self._optimizer.qaoa.sampler = Sampler(session=session, options=self._options)
                elif hasattr(self._optimizer, 'min_eigen_solver'):
                    self._optimizer.min_eigen_solver.sampler = Sampler(session=session, options=self._options)
