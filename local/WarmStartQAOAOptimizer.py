...
from qiskit_optimization.algorithms import SolutionSample, OptimizationResultStatus, OptimizationAlgorithm
...
class WarmStartQAOAOptimizer(MinimumEigenOptimizer):
    ...
    def __init__(
        ...
        self._pick_best_solution = pick_best_solution
    ...
    def solve(self, problem: QuadraticProgram) -> MinimumEigenOptimizationResult:
        ...
        first_index = 0 if self._pick_best_solution else 1
        if len(opt_result.samples) == 1:
            first_index = 0
        pre_solutions = opt_result.samples[first_index:num_pre_solutions]
        ...
        for pre_solution in pre_solutions:
            ...
            result = self._solve_internal(operator, offset, converted_problem, problem)
