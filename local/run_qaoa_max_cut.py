...
def get_filename(args):
    ...
    filename += f'AerSimulator_{args.optimizer}_{str(args.reps)}_MaxCut_{args.graph.replace(".pickle", "")}_{suffix}.json'
    return filename
...
def solve_and_store_result(qaoa):
    ...
    try:
        ...
        for sample in result.samples:
            x = ''.join(map(str, [int(i) for i in sample.x.tolist()]))
            samples[x].append(float(sample.probability))
