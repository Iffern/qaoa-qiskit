...
from qiskit_optimization.algorithms import OptimizationResult, OptimizationResultStatus, OptimizationAlgorithm
...
from MinimumEigenOptimizer import MinimumEigenOptimizationResult, MinimumEigenOptimizer
...
class RecursiveMinimumEigenOptimizer(OptimizationAlgorithm):
    ...
    def solve_hamiltonian(self, operator: OperatorBase, offset: float,
                          problem: QuadraticProgram = None) -> OptimizationResult:
        if problem is None:
        ...
        while problem_.get_num_vars() > self._min_num_vars:
            ...
            if problem_.get_num_vars() == problem_ref.get_num_vars() - 1:
                if hasattr(self._optimizer, '_pre_solver') \
                        and hasattr(self._optimizer._pre_solver, '_min_eigen_solver') \
                        and hasattr(self._optimizer._pre_solver._min_eigen_solver, '_filter'):
                    self._optimizer._pre_solver._min_eigen_solver._filter = None
                    self._optimizer._pre_solver._min_eigen_solver._num_initial_solutions = 10
            print(hasattr(self._optimizer, '_min_eigen_solver'))
            if (hasattr(self._optimizer, '_min_eigen_solver')
                    and hasattr(self._optimizer._min_eigen_solver, 'mixer')
                    and hasattr(self._optimizer._min_eigen_solver, 'initial_state')):
                self._optimizer._min_eigen_solver.mixer = None
                self._optimizer._min_eigen_solver.initial_state = None
